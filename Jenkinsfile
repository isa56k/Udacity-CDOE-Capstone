        ...
        steps{
          script{
            docker.withRegistry("https://" + registry, registryCredential) {
              def capstoneImage = docker.build(registry + ":${env.BUILD_ID}", "--build-arg CONTIMGVER=${env.BUILD_ID} -f Dockerfile .")
              capstoneImage.push()
        ...
        steps{
          withAWS(credentials: 'aws-creds', region: 'us-west-2') {
            sh """
              ...
              kubectl get pods
              kubectl get service
              kubectl set image deployments/capstone-web --record=true cdoe-capstone-proj=645851037944.dkr.ecr.us-west-2.amazonaws.com/cdoe-capstone-proj:${env.BUILD_ID}
            """
          }
          /*
          steps{    
